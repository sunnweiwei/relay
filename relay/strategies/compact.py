...
class Compact(BaseStrategy):
    ...
    def compact(
        ...
        summary = _summarize(responses, request, active)
...
def _summarize(
    ...
    history = deepcopy(active)
    protected = len(_protected_prefix(history))
    while True:
        call: dict[str, Any] = {
            "model": model,
            "input": [*history, {"role": "user", "content": CODEX_COMPACTION_PROMPT}],
            "store": False,
        }
        for key in (
            "instructions",
        ...
        try:
            ...
            if not _is_context_window_error(exc) or len(history) <= protected + 1:
                raise
            del history[protected]
